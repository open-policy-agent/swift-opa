...
/// Deprecated alias for ``AsyncBuiltin``, retained for source compatibility.
@available(*, deprecated, renamed: "AsyncBuiltin")
public typealias Builtin = AsyncBuiltin
...
public struct BuiltinRegistry: Sendable {
    ...
    /// Creates a registry by merging custom builtins with the default sync builtins.
    /// Custom builtins declared as `.sync` are available on both the async and sync VM paths;
    /// `.asyncOnly` builtins are available on the async path only.
    /// Throws `RegoError.ambiguousBuiltinError` if any custom builtin name conflicts with a default one.
    static func merging(customBuiltins: [String: BuiltinImpl]) throws -> BuiltinRegistry {
    ...
    /// Returns the best available implementation for `name`: sync if available, async-only otherwise.
    func lookup(_ name: String) -> BuiltinImpl? {
        if let sync = syncCallables[name] { return .sync(sync) }
        if let async = asyncCallables[name] { return .asyncOnly(async) }
        return nil
    }
