...
struct SyncSafePatcher {
    ...
    static func patch(policy: Bytecode.Policy, builtins: BuiltinRegistry) -> Bytecode.Policy {
        ...
        func analyzeFn(_ idx: Int) -> Bool {
            ...
            let safe = fn.blocks.allSatisfy {
                isBlockSyncSafe(
                    ...
                    callDynamicSyncSafe: true  // ignored in Phase 1; decided after all fnSafe computed
                )
            }
            fnSafe[idx] = safe
            return safe
        ...
        // callDynamic is safe iff every function in the policy is sync-safe:
        // whatever the runtime-resolved target is, it can only be one of these functions.
        let allFunctionsSyncSafe = fnSafe.allSatisfy { $0 == true }
        ...
        while let work = worklist.popLast() {
            ...
            let nested = patchBlock(
                ...
                callDynamicSyncSafe: allFunctionsSyncSafe
            )
        ...
        while pc < end {
            ...
            case .block1, .not:
                ...
                let safe = isBlockSyncSafe(
                    ...
                    callDynamicSyncSafe: callDynamicSyncSafe
                )
                if safe {
                    writeWord(&bytecode, at: pc, value: setInstrSyncSafeBit(word))
                }
            ...
            case .block:
                ...
                for i in 0..<n {
                    ...
                    let bSize = Int(decodeBlockSize(bSizeRaw))
                    let safe = isBlockSyncSafe(
                        bytecode: bytecode, strings: strings,
                    ...
                    if safe {
                        writeWord(&bytecode, at: ps + 4 + i * 8 + 4, value: setBlockSyncSafeBit(bSizeRaw))
                    }
            ...
            case .call:
                ...
                if !isBuiltinCall(rawFuncIdx) {
                    // User function: set bit 30 if sync-safe
                    let funcIdx = decodeFuncIndex(rawFuncIdx)
                    if functionSyncSafe(funcIdx) {
                        writeWord(&bytecode, at: ps + 4, value: setInstrSyncSafeBit(rawFuncIdx))
                    }
                }
    ...
    private static func isBlockSyncSafe(
        ...
        while pc < end {
            ...
            switch opcode {
            case .callDynamic:
                if !callDynamicSyncSafe { return false }
            case .call:
                let rawFuncIdx = bytecode.withUnsafeBytes {
                    $0.load(fromByteOffset: ps + 4, as: UInt32.self)
                }
