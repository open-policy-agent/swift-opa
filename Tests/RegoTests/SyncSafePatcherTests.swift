...
@Suite struct SyncSafePatcherTests {
    ...
    private func registryWithAsyncBuiltin(name: String) throws -> BuiltinRegistry {
        try BuiltinRegistry.merging(customBuiltins: [name: .asyncOnly({ _, _ in .null })])
    }
    ...
    @Test func testPlanWithCallDynamicIsSyncSafeWhenNoAsyncBuiltins() {
        // callDynamic is safe when all functions in the policy avoid async-only builtins.
        var enc = Encoder()
    ...
    @Test func testPlanWithCallDynamicIsNotSyncSafeWhenAsyncBuiltinsExist() throws {
        // callDynamic is unsafe when any function in the policy calls an async-only builtin.
        var fnEnc = Encoder()
        fnEnc.appendHeader(.call, payloadLength: 12)
        fnEnc.appendLocal(0)
        fnEnc.appendUInt32(0x8000_0000 | 0)  // builtin call, string index 0 = "my.async"
        fnEnc.appendUInt32(0)

        var planEnc = Encoder()
        planEnc.appendHeader(.callDynamic, payloadLength: 8)
        planEnc.appendLocal(1)
        ...
        let fn = Bytecode.Function(
            ...
            maxLocal: 0, bytecodeOffset: 0, bytecodeSize: fnEnc.bytes.count,
            blocks: [(offset: 0, size: fnEnc.bytes.count)])
        let p = makePolicy(
            plans: [singlePlan(offset: planOffset, size: planEnc.bytes.count)],
            functions: [fn], strings: ["my.async"], bytecode: bc)
        let patched = SyncSafePatcher.patch(
            policy: p, builtins: try registryWithAsyncBuiltin(name: "my.async"))
        #expect(patched.plans[0].syncSafe == false)
    }
